...
import { createSendCustomerEmail } from "./send-customer-email";
...
describe("createSendCustomerEmail", () => {
  it("sends to customer email with confirmation subject", async () => {
    const { _send, ...deps } = makeDeps();
    const send = createSendCustomerEmail(deps);
    await send(makeOrder());
    expect(_send).toHaveBeenCalledWith(expect.objectContaining({
      from: "orders@example.com",
      to: "alice@example.com",
      subject: expect.stringContaining("Your order is confirmed"),
    }));
  });

  it("includes orderId and amount in body", async () => {
    const { _send, ...deps } = makeDeps();
    const send = createSendCustomerEmail(deps);
    await send(makeOrder());
    expect(_send).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it("propagates Resend errors without swallowing", async () => {
    ...
    const sendEmail = createSendCustomerEmail(deps);
